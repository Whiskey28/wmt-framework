...
def extract_market_data(df):
    ...
    # 市场报告行数范围（Excel行号，需要转换为0索引）
    # 美国市场报告：301-334行 -> 索引300-333
    # 亚洲市场报告：335-368行 -> 索引334-367
    # 欧洲市场报告：369-402行 -> 索引368-401
    
    market_ranges = {
        '美国': {'start': 300, 'price_row': 304, 'func_row': 305, 'sales_row': 306, 'demand_row': 307, 'share_start': 328},
        '亚洲': {'start': 334, 'price_row': 338, 'func_row': 339, 'sales_row': 340, 'demand_row': 341, 'share_start': 362},
        '欧洲': {'start': 368, 'price_row': 372, 'func_row': 373, 'sales_row': 374, 'demand_row': 375, 'share_start': 396}
    }
    ...
    for market in MARKETS:
        ...
        for tech in TECHS[:2]:  # 只提取技术1和技术2
            ...
            # 提取价格（需要减1，因为Excel行号从1开始，pandas索引从0开始）
            price_row = range_info['price_row'] - 1 + offset
            # 提取功能数
            func_row = range_info['func_row'] - 1 + offset
            # 提取销量
            sales_row = range_info['sales_row'] - 1 + offset
            # 提取需求
            demand_row = range_info['demand_row'] - 1 + offset
            # 提取市场份额
            share_row = range_info['share_start'] - 1 + tech_offsets[tech] // 6
...
def extract_market_financial(df):
    ...
    # 各市场损益表行数范围（Excel行号，需要转换为0索引）
    income_statement_ranges = {
        '美国': {'start': 54, 'end': 82},   # 索引53-81
        '亚洲': {'start': 139, 'end': 167}, # 索引138-166
        '欧洲': {'start': 219, 'end': 242}  # 索引218-241
    }
    ...
    for market in MARKETS:
        for tech in ['技术1', '技术2']:
            if tech in market_data[market]:
                ...
                satisfy_rate = indicators['market'][market][tech].get('需求满足率', 0)
